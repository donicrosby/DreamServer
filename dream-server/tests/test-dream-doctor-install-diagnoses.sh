#!/usr/bin/env bash
# ============================================================================
# Dream Doctor install diagnosis tests
# ============================================================================
...
echo ""
echo "Dream Doctor Install Diagnosis Tests"
echo "===================================="
...
cwd=/tmp/not-the-dream-install-dir
...
if (cd "$ROOT_DIR" && bash scripts/dream-doctor.sh "$REPORT" >/dev/null 2>&1); then
    pass "dream-doctor runs with fixture install artifacts"
else
...
if jq -e '.diagnoses[] | select(.id == "DS-DOCKER-IMAGE-UNRESOLVED") | .evidence[0].source == "install-report-2999-01-01-000000.txt"' "$REPORT" >/dev/null; then
